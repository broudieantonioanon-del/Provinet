...
function PastePortalButton({ record, onUpdate }) {
  ...
  const handleClick = async () => {
    ...
    for (const line of lines) {
      if (/^Cliente:/i.test(line))         { cliente    = line.replace(/^Cliente:\s*/i, '').trim(); hasStructured = true; }
      else if (/^Cuenta:/i.test(line))     { cuenta     = line.replace(/^Cuenta:\s*/i, '').trim(); hasStructured = true; }
      else if (/^Tipo:/i.test(line))       { tipo       = line.replace(/^Tipo:\s*/i, '').trim(); hasStructured = true; }
      else if (/^Disponible:/i.test(line)) { disponible = line.replace(/^Disponible:\s*/i, '').trim(); hasStructured = true; }
      else if (/^Total:/i.test(line))      { total      = line.replace(/^Total:\s*/i, '').trim(); hasStructured = true; }
      else if (/^Ref\./i.test(line))       { ref        = line.replace(/^Ref\.\s*/i, '').trim(); hasStructured = true; }
    }

    if (!hasStructured && text.trim()) {
      cliente = text.trim().toUpperCase();
    }
